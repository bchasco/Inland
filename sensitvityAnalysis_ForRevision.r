...
sensitivityParNames = c('klAlpha', 'mass', 'abund', 'Age0', 'cFrac', 'smoltLength', 'alphaOneill', 'betaOneill')
nSim = 25
cvs = array(NA, c(3,length(sensitivityParNames),length(sensitivityParNames)+2,nSim))
...
for(sensitivityCV in 1:3)
{
  for(sensitivityPar in 1:length(sensitivityParNames))
  {
    for(sim in 1:nSim)
    {
      ...
      cvs[sensitivityCV,sensitivityPar,,sim] = c(kleiberErr,predWtErr, predAbundanceErr, age0Err, chinookFracErr, lenErr, oneillAlphaErr, oneillBetaErr, sensitivityNumbers, sensitivityBiomass)
    }
  }
}
