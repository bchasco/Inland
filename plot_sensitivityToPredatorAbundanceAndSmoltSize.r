png("plot_sensitivityToPredatorAbundanceAndPredatoryActivity.png", width=480, height=780, pointsize=12)
...
for(pa in 1:3)
  for(am in 1:3)
  {
    ...
    par(fig=c(0.1+(pa-1)/3*0.85,0.1+(pa)/3*0.85,0.05+(ssm-1)/3*0.9,0.05+(ssm)/3*0.9), mai=c(0,0,0,0))
    barplot((sensivityNumbers[pa,ssm,,])/1000000, 
    ...
    if(pa==2 & ssm==2) text(23,25,"Base case", cex=1.4)
    icnt = icnt + 1
